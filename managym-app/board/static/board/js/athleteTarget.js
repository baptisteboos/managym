...
function validateScoreColor(selector) {
  if (parseFloat($(selector).text()) >= 90) {
    $(selector).css('color', 'green'); // good score (>=90%)
  } else if (parseFloat($(selector).text()) < 80) {
    $(selector).css('color', 'red'); // bad score (<80%)
  } else {
    $(selector).css('color', '#b9b226'); // average score (>=80% - <90%)
  }
}
...
function handlerTargetResults (selector, init=false) { 
  ...
  if (!init) {
    ...
    $.ajax({
      type: "POST",
      url: $('#accordionTargetResults').attr('data-update-target-url'),
      data: {
        'event_id': event_id,
        ...
        'rsv': rsv,
        'rex': rex,
        'target': parseFloat($(tbody).find('td.target-total').text()),
        'result': parseFloat($(tbody).find('td.result-total').text())
      },
      dataType: 'json',
      success: function () {
      },
      error: function (error) {
        console.log("ERROR: Target not update!");
        console.log(error);
      }
    });
  }
